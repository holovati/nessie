...
uint8_t bus_data::read8(uint16_t a_addr)
{
    bus_device_t dev = m_device_map[a_addr >> PAGE_SHIFT];

    if (dev)
    {
        return dev->m_ops->read8(dev, a_addr - dev->m_base);
    }
...
uint16_t bus_data::read16(uint16_t a_addr)
{
    bus_device_t dev = m_device_map[a_addr >> PAGE_SHIFT];
    ...
    {
        a_addr -= dev->m_base;
        return (((uint16_t)dev->m_ops->read8(dev, a_addr + 1)) << 8) | ((uint16_t)dev->m_ops->read8(dev, a_addr));
    }

    return 0xFFFF;
}

void bus_data::write8(uint16_t a_addr, uint8_t a_value)
{
    bus_device_t dev = m_device_map[a_addr >> PAGE_SHIFT];

    if (dev)
    {
        dev->m_ops->write8(dev, a_addr - dev->m_base, a_value);
    }
}

// This is a convienience function, in reallity we just do two 8 bit writes
void bus_data::write16(uint16_t a_addr, uint16_t a_value)
{
    bus_device_t dev = m_device_map[a_addr >> PAGE_SHIFT];
    ...
    {
        a_addr -= dev->m_base;
        dev->m_ops->write8(dev, a_addr + 1, a_value >> 8);
        dev->m_ops->write8(dev, a_addr, a_value & 0xFF);
    }
}
